...
class DimePipeline:
    ...
    # Database setup
    conn = sqlite3.connect('dime.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `dime`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM dime WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO dime (date, title, link, content)"
